...
Packages in this monorepo follow the roadmap version (currently **v0.2.1**). Path repositories use `@dev` during development; tagged releases (`vX.Y.Z` on the repository) align all package `composer.json` versions. See [docs/RELEASE.md](docs/RELEASE.md) for the full release checklist.
